...
coords = {"groups": group_names, "obs": np.arange(n_obs)}
...
summary = az.summary(
    idata, var_names=["mu_alpha", "sigma_alpha", "mu_beta", "sigma_beta", "sigma", "alpha", "beta"]
)
print("\nParameter Summary:")
print(summary)
...
if divergences > 0:
    ...
    print(
        "   This is common in hierarchical models - non-centered parameterization already applied"
    )
...
az.plot_trace(
    idata, var_names=["mu_alpha", "sigma_alpha", "mu_beta", "sigma_beta", "sigma"], axes=axes
)
plt.tight_layout()
...
axes[0].axvline(
    idata.posterior["mu_alpha"].mean().item(), color="red", linestyle="--", label="Population mean"
)
axes[0].legend()
...
axes[1].axvline(
    idata.posterior["mu_beta"].mean().item(), color="red", linestyle="--", label="Population mean"
)
axes[1].legend()
...
for i, g in enumerate(new_groups):
    print(
        f"{group_names[g]:<10} {new_X[i]:<10.2f} {y_pred_mean[i]:<15.3f} {y_pred_hdi[i, 0]:<15.3f} {y_pred_hdi[i, 1]:<15.3f}"
    )
